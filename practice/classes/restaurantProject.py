...
class Franchise:
    ...
    def __repr__(self):
        return "This store is located at {ad}.".format(ad=self.address)
...
class Menu:
    ...
    def __repr__(self):
        return "{name} menu is avaliable from {start} to {end} O'clock.".format(name=self.name, start=self.start_time, end=self.end_time)
...
early_bird = Menu('early bird', {'salumeria plate': 8.00, 'salad and breadsticks (serves 2, no refills)': 14.00,
                  'pizza with quattro formaggi': 9.00, 'duck ragu': 17.50, 'mushroom ravioli (vegan)': 13.50, 'coffee': 1.50, 'espresso': 3.00, }, 15, 18)
...
print(flagship_store.avaliable_menus(12))
print(flagship_store.avaliable_menus(17))

pastaBiz = Business("Basta Fazoolin' with my Heart", [
                    flagship_store, new_installment])

arepas_menu = Menu('Arepas', {'arepa pabellon': 7.00, 'pernil arepa': 8.50,
                   'guayanes arepa': 8.00, 'jamon arepa': 7.50}, 10, 20)
arepas_place = Franchise("189 Fitzgerald Avenue", [arepas_menu])
arepaBiz = Business("Take a' Arepa", [arepas_place])
